...
// this time making this bucket component which can communicate with the backend using node express mongodb
...
const Bucket = () => {
  ...
  const handleAddItem = async () => {
    ...
    if (bucketItem.trim() !== '' && moment(targetDate, 'YYYY-MM-DD', true).isValid()) {
      ...
      try {
        const response = await axios.post(`http://localhost:4000/users/${userID}/add`, newItem);
  ...
  const handleRemoveItem = async (item) => {
    try {
      const response = await axios.delete(`http://localhost:4000/users/${userID}/remove`, { data: { text: item.text } });
      setBucketList(response.data);
    } catch (error) {
  ...
  const postBucketList = async () => {
    ...
    try {
      const response = await axios.put(`http://localhost:4000/users/${userID}/update`, { bucket: bucketList });
      setAlertMessage("Item data sent to the server ✔");
    } catch (error) {
      console.error('Error updating bucket list:', error);
